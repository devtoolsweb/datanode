import { DataNode, IDataNode } from '../lib'
...
test('makePath', () => {
    ...
    ;(parent.makePath('x/y/1/2/Name With Spaces') as IDataNode).value = 100
...
test('addChild', () => {
    ...
    parent.on('addChild', event => {
        if ((event.child as IDataNode).getInt() > 0) {
            n++
        }
    })
    parent.addChild(new DataNode({
        value: '0',
        name: 'child 1'
    }))
    parent.addChild(new DataNode({
...
test('removeChild', () => {
    ...
    let c = parent.getChildAt(5) as IDataNode
    expect(c.value).toBe(5)
    parent.removeChild(c)
    c = parent.getChildAt(5) as IDataNode
...
test('addSuccessorNode', () => {
    ...
    expect(() => parent.addSuccessorNode('../s3', new DataNode({ name: 'successor 4' }))).toThrow(Error)
})
...
test('emitEvent', async () => {
    ...
    const p = Promise.all([
        ...
        new Promise<void>(resolve => {
            (parent.getNodeByPath(path) as IDataNode).on('change', () => {
                n++
                resolve()
            })
        })
    ])
    ;(parent.getNodeByPath(path) as IDataNode).value = 123
    await p
    expect(n).toBe(2)
})
