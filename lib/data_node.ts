 ...
 */
import { BaseClass, BaseClassFlags, IBaseClass, IBaseClassOpts, INStructChild, INStructChildConstructor, INStructContainer, INStructContainerConstructor, NStructChildMixin, NStructContainerMixin } from '@devtoolsweb/essentials'
import { Constructor } from 'type-fest'
import { EventEmitterMixin, IBaseEvents, ITypedEvent, ITypedEventEmitter, ITypedEventOpts, TypedEvent } from '@devtoolsweb/event-emitter'
...
export type DataNodeNStructChild = Constructor<INStructChild> &
...
export const MixinDataNodeNStructChild = (base: Constructor<IBaseClass>): DataNodeNStructChild & INStructChildConstructor<INStructChild> =>
    NStructChildMixin<Constructor<IBaseClass>>(base)

export const MixinDataNodeNStructContainer = (base: Constructor<INStructChild & IBaseClass>): DataNodeNStructContainer =>
    NStructContainerMixin<
    IDataNode,
    Constructor<INStructChild> & Constructor<IBaseClass>
    >(base)

export const MixinDataNodeEventEmitter = (base: DataNodeNStructContainer): DataNodeNStructContainer &
Constructor<ITypedEventEmitter<IDataNodeEvents>> =>
    EventEmitterMixin<IDataNodeEvents, DataNodeNStructContainer>(base)

export const BaseDataNodeConstructor = MixinDataNodeEventEmitter(MixinDataNodeNStructContainer(MixinDataNodeNStructChild(BaseClass)))
...
export class DataNode extends BaseDataNodeConstructor implements IDataNode {
    ...
    constructor (p: IDataNodeOpts) {
        super({
            ...p,
            name: DataNode.verifyName(p.name) as string
        })
    ...
    set value (value: DataNodeValue) {
        if (this.$value !== value) {
            this.$value = value
            this.emitEvent(new DataNodeEvent({
                confirmCallback: p => {
                    this.$value = p.result as DataNodeValue
                },
                origin: this,
                type: 'change'
            }))
    ...
    getDate (): Date {
        if (this.$value instanceof Date) {
            ...
            if (isNaN(d.getTime())) {
                throw new Error(`DN0002: The data node '${this.fullPath}' does not contain a value of type Date`)
            }
            return d
        }
    }
    ...
    getFloat (): number {
        ...
        if (typeof v === 'number') {
            ...
            if (isNaN(n)) {
                throw new Error(`DN0005: The data node '${this.fullPath}' does not contain a value of type Number`)
            }
            return n
        }
    }
    ...
    getRelativePath (node: IDataNode, target: IDataNode): string {
        if (node.root !== target.root) {
            throw new Error(`DN0006: Date nodes must belong to one tree: '${node.fullPath}', '${target.fullPath}'`)
        }
    ...
    insertChild (child: IDataNode, index = Infinity): this {
        ...
        return this.emitEvent(new DataNodeEvent({
            child,
            childIndex,
            origin: this,
            type: 'addChild'
     ...
     *
     * @param {string} path Data node path
     * @param {DataNodeCreator} createNode Creator
     * @returns {IDataNode} New node
     */
    makePath (path: string, createNode?: DataNodeCreator): IDataNode {
        return this.walkPath(
            ...
            (node: IDataNode | null, pathParts: string[] = []): IDataNode => {
                ...
                if (createNode) {
                    ...
                    if (!newNode) {
                        throw new Error(`DN0008: Data node builder returns null: ${pp.join(DataNode.pathSeparator)}`)
                    }
                    return newNode
                }
                else {
    ...
    toString (): string {
        const parts: string[] = []
    ;(function dump (node: IDataNode, indent = 0) {
            const path = `${node.fullPath}${
                node.isLink ? ` ~ @${node.realPath}` : ''
            }`
            parts.push(`${' '.repeat(indent)}${path}${
     ...
     *
     * @param {string} path Data node path
     * @param {DataNodeVisitor} visit Visitor
     * @returns {IDataNode} Data node or null
     */
    walkPath (path: string, visit: DataNodeVisitor): IDataNode | null {
        if (path.charAt(0) === ' ' || path.charAt(path.length - 1) === ' ') {
            throw new Error(`DN0009: Path to data node must not be enclosed in spaces: "${path}"`)
        }
        let node: IDataNode | null = this as IDataNode
        if (path.length > 0) {
            ...
            for (let i = 0; i < p.length; i++) {
                ...
                if (name === '') {
                    ...
                    fp.push(name)
                    const child = node.findChildNode(name)
